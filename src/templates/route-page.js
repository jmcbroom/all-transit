...
const RouteTrips = ({ trips, feedIndex }) => {
  let filteredTrips = _.filter(trips, t => { return t.direction === 0 && feeds[feedIndex - 1].services[t.service] === "weekday"}).sort((a, b) => {
    return (a.stopTimes[0].arrivalTime.hours * 60 + a.stopTimes[0].arrivalTime.minutes) - (b.stopTimes[0].arrivalTime.hours * 60 + b.stopTimes[0].arrivalTime.minutes) 
  })
  let timepointList = filteredTrips.sort((a, b) => {
    return b.stopTimes.map(s => s.timepoint).reduce((acc, val) => acc + val) - a.stopTimes.map(s => s.timepoint).reduce((acc, val) => acc + val)
  })[0].stopTimes.filter(st => { return st.timepoint === 1})
        ...
        <tr key={tr.id}>
          {timepointList.map((t, i) => {
            ...
            if(filtered.length === 0){
              ...
              let st = filtered[0]
              return <td key={st.stop.stopId}>{st.arrivalTime.hours} {st.arrivalTime.minutes}</td>
            }
          })}
        </tr>
      )))}
